...
class PDFVectorStore:
    ...
    def load(self, save_dir="vectorstore"):
        ...
        try:
            ...
            logging.info(f"Vectorstore loaded successfully from {save_dir}")
            return True
        except Exception as e:
            logging.error(f"Error loading vectorstore: {e}")
            return False

    def query(self, text_query="", image_path=None, top_k=1) -> List[tuple]:
        ...
        try:
            ...
            for idx in indices[0]:
                if idx != -1 and idx < len(self.pdf_mapping):
                    result = (self.pdf_mapping[idx], self.page_numbers[idx])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="PDF Vector Store Operations")
    parser.add_argument(
        "--mode",
        choices=["create", "query", "append"],
        required=True,
        help="Operation mode: create new index, query existing, or append",
    )
    parser.add_argument("--query_text", type=str, help="Text to search for")
    parser.add_argument("--image_path", type=str, help="Path to image for search")
    parser.add_argument(
    ...
    try:
        if args.mode == "create":
            ...
            logger.info("Loading vector store for query...")
            if vectorstore.load():
                results = vectorstore.query(
                    text_query=args.query_text,
            ...
            logger.info("Loading vector store for appending...")
            if vectorstore.load():
                vectorstore.append_pdfs()
                vectorstore.save()
                logger.info("Successfully appended new documents")
            else:
                logger.error("Failed to load vector store")
